...
export class UpdateNotifications {
  ...
  updateNotify(){
    ...
    this.http.putApi(`http://localhost:8080/notify/update`,this.notify).subscribe((updateNotifyRes:any)=>{
      if(!this.validNotify()){
        return;
      }
  ...
  saveNotify(){
    ...
    this.http.putApi(`http://localhost:8080/notify/update`,this.notify).subscribe((addNotifyRes:any)=>{
