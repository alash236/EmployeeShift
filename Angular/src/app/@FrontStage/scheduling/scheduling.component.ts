...
punchIn(): void {
  ...
  console.log('[punchIn] nowTime =', nowTime, 'workDate =', workDate);

  const normalize = (t: string) => {
    if (!t) return '';
    const p = t.trim().split(':').map(Number);
    const [hh, mm, ss = 0] = p.length === 2 ? [p[0], p[1], 0] : p;
  ...
  const shifts = this.acceptedSlots(raw)
    .map(s => ({
  ...
  this.http.get<any>('http://localhost:8080/all').subscribe({
    next: (all) => {
      const list: any[] = all?.clockDateInfoResList ?? all?.data ?? all?.list ?? [];
      console.log('[punchIn] /all size =', list.length);

      const todayRec =
        list.find((r: any) =>
          String(r.employeeId).trim() === employeeId &&
          String(r.workDate).slice(0, 10) === workDate
        ) || null;
      ...
      dialogRef.afterClosed().subscribe((refresh) => {
        console.log('[punchIn] dialog closed, refresh =', refresh);
        if (refresh) this.loadClockData();
      });
    },
    error: (err) => {
      console.error('[punchIn] /clock/all error:', err);
      this.dialog.open(ErrorDialogComponent, { data: { message: '無法讀取打卡紀錄' } });
    }
  });
}
